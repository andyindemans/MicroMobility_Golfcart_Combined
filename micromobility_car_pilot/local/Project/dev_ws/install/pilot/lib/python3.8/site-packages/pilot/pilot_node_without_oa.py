...
class PilotNodeWithoutOa(Node):
    ...
    def __init__(self):
        ...
        self.pid = PID(1, 0, 0, setpoint=0, output_limits=(-180, 180))
        logger.debug("Pilot running")

    def next_heading_callback(self, msg):
        logger.debug('Next heading: "%s"' % msg.data)

        self.next_heading = msg.data

        #self.pid.setpoint = self.next_heading
        #control = self.pid(self.next_heading)

        msg = CarInstructions()
        msg.distance = self.distanceToNode
        msg.steering_range = self.range
        msg.throttle = self.throttle
        msg.angle = self.next_heading

        logger.debug("pid controller: " + str(self.next_heading))
